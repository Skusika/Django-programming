...
class MovieModelTestcase(TestCase):
    ...
    def test_movie_change_value(self):
        # Проверка изменения записи из бд
        self.print_info('Start test_movie_change_value')
        mask = Movie.objects.get(name='Mask')
        mask.name = "Bob"
        mask.save()
        self.assertEqual(mask.name, "Bob")
        self.print_info('Finish test_movie_change_value')

    def test_movie_delete_value(self):
        # Проверка удаления записи из бд
        self.print_info('Start test_movie_delete_value')
        mask = Movie.objects.get(name='Mask')
        mask.delete()
        self.assertEqual(Movie.objects.count(), 2)
        self.print_info('Finish test_movie_delete_value')
